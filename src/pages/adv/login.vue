  ...
  <div class="login-adm-content">
    ...
    <div class="login-wrapper">
      ...
      <div class="login-body">
        <div class="login-title">广告主账号</div>
...
export default {
  ...
  methods: {
    ...
    subsignin () {
      ...
      this.$http.post('/v1/login/adv', params).then((res) => {
        this.loadings = false;
        let data = res.body;
        if(data.ret!=1){
        ...
        if (path === config.login) {
          this.$router.replace({path: '/adv/index'});
        } else {
    ...
    changeCode () {
      let url = apiUtil.url("/v1/adv/verificationcode?");
      url += 't=' + (+(new Date()));
